...
export default async function DashboardPage() {
  ...
  type ActivityItem =
    | { kind: 'overdue'; reminder: NonNullable<typeof reminders>[number] }
    | { kind: 'expiring'; doc: NonNullable<typeof documents>[number] }
    | { kind: 'upcoming'; reminder: NonNullable<typeof reminders>[number] }

  const overdueItems: ActivityItem[] = (reminders?.filter(r =>
    r.due_date && r.due_date < todayStr && !r.completed
  ) || []).map(r => ({ kind: 'overdue', reminder: r }))

  const todayItems: ActivityItem[] = (reminders?.filter(r =>
    r.due_date && r.due_date === todayStr && !r.completed
  ) || []).map(r => ({ kind: 'upcoming', reminder: r }))

  const expiringItems: ActivityItem[] = (documents?.filter(doc => {
    if (!doc.expires_at) return false
    const expiresDate = new Date(doc.expires_at)
    return expiresDate <= thirtyDaysFromNow && expiresDate >= today
  }) || []).map(doc => ({ kind: 'expiring', doc }))

  const upcomingItems: ActivityItem[] = (reminders?.filter(r =>
    ...
    <div className="p-8 relative">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          {/* Colonne gauche : À traiter + Documents récents + Actions rapides */}
              ...
              <div className="glass-card rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shadow-md ${
                    activityItems.some(i => i.kind === 'overdue')
                      ? 'bg-gradient-to-br from-red-500 to-red-600 shadow-red-500/30'
                      : 'bg-gradient-to-br from-indigo-500 to-violet-600 shadow-indigo-500/20'
                  }`}>
                    <Bell className="w-4 h-4 text-white" />
                  </div>
                  <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">À traiter</h2>
                ...
                {activityItems.length > 0 ? (
                  <div className="space-y-2">
                    {activityItems.map((item) => {
                      if (item.kind === 'overdue') {
                        ...
                        const isLate = reminder.due_date && reminder.due_date < todayStr
                        return (
                          <Link key={`overdue-${reminder.id}`} href={`/topics/${reminder.topic_slug}`}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{reminder.title}</span>
                              ...
                              <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                                ...
                                {reminder.due_date && (<><span>•</span><span className="text-red-600 dark:text-red-400">{new Date(reminder.due_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span></>)}
                              </div>
                            </div>
                      ...
                      if (item.kind === 'expiring') {
                        const doc = item.doc
                        return (
                          <Link key={`expiring-${doc.id}`} href={`/topics/${doc.topic_slug}`}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="px-1.5 py-0.5 bg-amber-500 text-white text-[10px] font-semibold rounded-full flex-shrink-0">Expire bientôt</span>
                              </div>
                              {doc.expires_at && (
                                <div className="text-xs text-amber-600 dark:text-amber-400">
                                  {getTopicTitle(doc.topic_slug)} • Expire le {new Date(doc.expires_at).toLocaleDateString('fr-FR')}
                                </div>
                              )}
                            </div>
                            <ArrowRight className="w-4 h-4 text-zinc-400 flex-shrink-0 group-hover:text-amber-500 transition-colors" />
                          </Link>
                        )
                      }
                      const reminder = item.reminder
                      return (
                        <Link key={`upcoming-${reminder.id}`} href={`/topics/${reminder.topic_slug}`}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{reminder.title}</div>
                            <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                              <span>{getTopicTitle(reminder.topic_slug)}</span>
                              {reminder.due_date && (<><span>•</span><span className="text-indigo-600 dark:text-indigo-400">{new Date(reminder.due_date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span></>)}
                            </div>
                          </div>
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 flex items-center justify-center flex-shrink-0">
                        <CheckCircle2 className="w-4 h-4 text-indigo-400 dark:text-indigo-500" />
                      </div>
                      <div>
                        <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Tout est à jour !</p>
                        <p className="text-xs text-zinc-500 dark:text-zinc-400">Aucun rappel urgent ni document expirant.</p>
                      </div>
                    </div>
                    <Link href="/topics"
                      className="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-lg shadow-sm hover:shadow-md hover:shadow-indigo-500/25 transition-all duration-150 text-xs font-medium"
                    >
              ...
              <div className="glass-card rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Documents récents</h2>
                  </div>
                  <Link href="/topics" className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-medium flex items-center gap-1">
                ...
                {documents && documents.length > 0 ? (
                  <div className="space-y-2">
                    {documents.slice(0, 5).map((doc) => (
                      <Link key={doc.id} href={`/topics/${doc.topic_slug}`}
                        className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20 transition-colors border border-indigo-100/50 dark:border-indigo-800/30 group"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{doc.name}</div>
                  ...
                  <div className="flex items-center gap-3 text-zinc-500 dark:text-zinc-400">
                    <div className="w-8 h-8 rounded-xl bg-zinc-50 dark:bg-zinc-800/60 flex items-center justify-center flex-shrink-0">
          ...
          {/* Colonne droite : Calendrier uniquement */}
        ...
        <AnimateOnScroll className="mt-6">
          <div className="glass-card rounded-2xl p-4 overflow-hidden relative">
            ...
            <div className="grid grid-cols-3 gap-3">
              <Link href="/topics"
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-indigo-50/60 dark:hover:bg-indigo-900/20 transition-colors border border-zinc-200 dark:border-zinc-700/50 group"
              >
                <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-sm shadow-indigo-500/20 group-hover:shadow-md group-hover:shadow-indigo-500/30 transition-shadow flex-shrink-0">
                  <Plus className="w-4 h-4 text-white" />
                </div>
                <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Créer un rappel</span>
              </Link>
              <Link href="/topics"
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-indigo-50/60 dark:hover:bg-indigo-900/20 transition-colors border border-zinc-200 dark:border-zinc-700/50 group"
              >
